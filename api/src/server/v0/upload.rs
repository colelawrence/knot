...
pub fn get_url(req: &HttpRequest<State>) -> FutureResponse<HttpResponse> {
    ...
    use crate::{access_guard, SigninState};
        ...
        access_guard(&req)
            ...
            .and_then(move |session: UserSession| {
                store_actor
                    ...
                    .map_err(|_| error::ErrorInternalServerError("Failed to get put url"))
                    .flatten()
                    ...
                    ensure_ownership_of_pending(&session.person, &req.key)
                        ...
                        .and_then(move |_| {
                            store_actor
                                ...
                                .map_err(|_| error::ErrorInternalServerError("Failed to finalize object"))
                                .flatten()
